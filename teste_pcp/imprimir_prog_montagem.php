...
$end_final   =  "Imprimir PCP Previsão Montagem Diária.pdf"; 
//TIPO DO PDF GERADO                                      
//F-> SALVA NO ENDEREÇO ESPECIFICADO NA VAR END_FINAL     
$tipo_pdf    =  "F";                          
//TÍTULO DO RELATÓRIO                                     
$titulo      =  "Imprimir PCP Previsão Montagem Diária";
...
$dias_inicial = 0;	
$dias_prog_montagem = $dias_prog_montagem - 1; 
...
//	$pdf->Cell(40, -15, "Data Impressão - $data_emissao_hoje" , 0, 0, 'R');    
	$pdf->Cell(180, -15, "Impressão PCP Programação Montagem" , 0, 0, 'C');
//  $pdf->Cell(25, -15, "Pág. 1 de $paginas", 0, 0, 'R');
                    	

// FOR SOMAR DIAS	------------------------------------------------
	for ( $dias=$dias_inicial; $dias<=$dias_prog_montagem; $dias++ )	{
  

// DESCOBRI DIA, MES E ANO DA DATA PROGR. DIARIA  ------------------------------------------------
//echo "<br>"; echo $f_data_prog_diaria;
$dia = substr($f_dt_prog_montagem, -2); 
$mes = substr($f_dt_prog_montagem, -5,2); 
$ano = substr($f_dt_prog_montagem, -10,4);
...
// Monta Saída ----------------------------------------------------------------
	$dt_prog_montagem_nova = $dia."/".$mes."/".$ano;   
	$dt_prog_montagem_nova_db = $ano."/".$mes."/".$dia; 
...
//MONTA O SUB-TITULO 
   $pdf->Ln(-2); 
   $pdf->SetFont("Arial", "B", $fonte_cabecalho);
   $pdf->Cell(45, 8, "Data Montagem $dt_prog_montagem_nova - $dia_semana", 0, 1, 'L');   
...
//CONECTA COM O MYSQL
	...
	$query = "select * from pcp where dt_prog_montagem='$dt_prog_montagem_nova_db' $f_data_emissao_db $f_num_os_db $f_num_proposta_db $f_nome_cliente_db $f_oc_obra_db $f_mercado_db $f_estado_db $f_data_entrega_db $f_local_venda_db $f_fornecimento_motor_db $f_descr_vent_db $f_modelo_db $f_tamanho_db $f_arranjo_db $f_classe_db $f_rotacao_db $f_gab_db $f_pintura_db $f_construcao_db $f_qt_db $f_valor_uni_db $f_valor_total_db $f_obs_db $f_data_motor_recebido_db $f_reprogramacao_db $f_baixa_db $f_data_baixa_db ORDER BY data_entrega";
...
$data_entrega = ($dia_data_entrega."/".$mes_data_entrega."/".$ano_data_entrega); 
   ...
   $pdf->Cell(20, 4, $descr_vent, 1, 0, 'C');  
   $pdf->Cell(8, 4, $qt, 1, 0, 'C');
   $pdf->Cell(12, 4, $modelo, 1, 0, 'C');    
   $pdf->Cell(10, 4, $tamanho, 1, 0, 'C'); 
   $pdf->Cell(8, 4, $arranjo, 1, 0, 'C');  
   $pdf->Cell(8, 4, $rotacao, 1, 0, 'C'); 
   $pdf->Cell(8, 4, $gab, 1, 0, 'C');  
   $pdf->Cell(8, 4, $construcao, 1, 0, 'C'); 
   $pdf->Cell(15, 4, $pintura, 1, 0, 'C');
   
   $pdf->Cell(8, 4, $mat1, 1, 0, 'C');
   $pdf->Cell(8, 4, $mat2, 1, 0, 'C');
   $pdf->Cell(8, 4, $mat3, 1, 0, 'C');
   $pdf->Cell(8, 4, $status_montagem, 1, 1, 'C');
...
if ( $f_dt_prog_montagem == "" ) {
   $pdf->Ln(6);    
   $pdf->SetFont("Arial", "B", $fonte_erro);
   $pdf->Cell(180, 10, "VOCÊ NÃO SELECIONOU A DATA PROGRAMADA MONTAGEM", 1, 1, 'C'); 
...
//MONTA O RODAPE              
	...
	$pdf->Ln(16);
	$pdf->Cell(250, -15, "Data Impressão - $data_emissao_hoje - $hora_atual" , 0, 0, 'R');    
