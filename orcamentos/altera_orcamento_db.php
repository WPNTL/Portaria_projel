...
$libhorafim = $linha["libhorafim"];
			...
			// usuário tiver permissão de alteração nas data e hora da solicitação
if ( $libalteratempo == "" ) { 
...
$resultado = mysql_query($sql) or die ("Não foi possível realizar a consulta ao banco de dados");  
	...
	elseif ( $tempo_seg_novo <> "" ) // se tiver valor no campo tempo_seg, alterar somente dados
...
$resultado = mysql_query($sql) or die ("Não foi possível realizar a consulta ao banco de dados"); 
	...
	}	  // se tiver valor no campo tempo_seg, alterar somente dados
			...
			{ // usuário não ten permissão de alteração nas datas da solicitação
$sql = "UPDATE n_orcamento SET
cliente='$cliente_novo', 
mercado='$mercado_novo', 
contato='$contato_novo', 
