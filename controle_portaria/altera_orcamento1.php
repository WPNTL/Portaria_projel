...
<html>
<head>
<title> Alterar Registro Portaria </title>
...
$sql = "SELECT * FROM controle WHERE id='$fid'";
$resultado = mysql_query($sql) or die ("Não foi possível realizar a consulta ao banco de dados");
while ($linha=mysql_fetch_array($resultado)) {
$id = $linha["id"]; 
...
<td class=left>
<input class=left name=empresa_novo maxLength=20 size=20 value="<?echo $empresa?>"
...
<td class=left>
<input class=left name=nome_novo maxLength=50 size=20 value="<?echo $nome?>"
...
var arvoren_nota = new Array(<?= $palavrasn_nota ?>);
