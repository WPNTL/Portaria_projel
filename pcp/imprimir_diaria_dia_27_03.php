...
$dias_inicial = 0;	$dias_prog = $dias_prog - 1; 
//PAGINA DO PDF
...
// CRIAR PÁGINA NO PDF   
	...
	$pdf->Cell(130, -15, "Impressão PCP Diária" , 0, 0, 'C');
...
$data_entrega = $dados["data_entrega"]; $num_os = $dados["num_os"]; $item = $dados["item"]; 

$nome_cliente = $dados["nome_cliente"];$oc_obra = $dados["oc_obra"]; $mercado = $dados["mercado"]; $estado = $dados["estado"];  $fornecimento_motor = $dados["fornecimento_motor"];

$descr_vent = $dados["descr_vent"]; $qt = $dados["qt"]; $modelo = $dados["modelo"]; $tamanho = $dados["tamanho"]; $arranjo = $dados["arranjo"]; $rotacao = $dados["rotacao"]; $gab = $dados["gab"]; $construcao = $dados["construcao"];  $pintura = $dados["pintura"]; 

$dia_data_entrega = substr($data_entrega, -2);   $mes_data_entrega = substr($data_entrega, -5,2);  $ano_data_entrega = substr($data_entrega, -10,4); $data_entrega = ($dia_data_entrega."/".$mes_data_entrega."/".$ano_data_entrega); 
