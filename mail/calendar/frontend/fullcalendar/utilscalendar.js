...
function saveEvent(){
    $("#saveEvent").click(
        function(){
   	    if($('[name=summary]').val() == ''){
		$('[name=summary]').addClass('emptyField')
		.attr('title',"TESTE TESTE");
		return;
	    }else{
		$('[name=summary]').removeClass('emptyField');	
	    }
...
function dialogCreateEvent(edit,id,start,end){                
    ...
    if(edit){
		...
		if(arrayEvent['alarm_period'] != null){
			$('[name=periodAlert] option[value=' +  arrayEvent['alarm_period']  +  ']').attr('selected','selected')
		}
