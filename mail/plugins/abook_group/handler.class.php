...
class HandlerGroup {
	...
	public function updateNickname($oldNick,$newnickname){
		$this->sql = "update addressgroups set nickname = '$newnickname' where nickname = '$oldNick' and owner = '$this->userOwner'";
		$res = $this->db->query($this->sql);
        if (PEAR::isError($res)){
           die($res->getMessage());
        } 
    ...
    public function selectMails($group) {
        $this->sql = "select a.email from address as a left join addressgroups as b on b.nickname = a.nickname where b.addressgroup = '$group' and a.owner = b.owner";
		// and b.owner ='" . $this->userOwner . "'";
		//echo $this->sql;
