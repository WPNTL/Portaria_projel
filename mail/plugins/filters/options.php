    ...
    if ($complete_post) {
        ...
        if ($action != 'edit') {
			...
			/*else{
 	 	    ...
 	 	    $select = "select * from userprefs where prefkey = 'filter$theid' and \"user\" = '$username'";
   ...
   echo html_tag( 'table',
    ...
    for ($i=0, $num = count($filters); $i < $num; $i++) {
        ...
        echo html_tag( 'tr', '', '', $clr ) .
                   html_tag( 'td',
                       '<small>' .
                       "[<a href=\"options.php?theid={$filters[$i]['id']}&amp;action=edit\">" . _("Edit") . '</a>]'.
                       '</small>' ,
                   'left' ) .
                   html_tag( 'td',
                       '<small>' .
                       "[<a href=\"options.php?theid={$filters[$i]['id']}&amp;action=delete\">" . _("Delete") . '</a>]'.
                       '</small>' ,
                   'left' );
        ...
        if ($num > 1) {
            ...
            if (isset($filters[$i + 1])) {
                echo "<a href=\"options.php?theid={$filters[$i]['id']}&amp;action=move_down\">" . _("Down") . '</a>';
                if ($i > 0) {
                    echo '&nbsp;|&nbsp;';
                }
            }
            if ($i > 0) {
                echo "<a href=\"options.php?theid={$filters[$i]['id']}&amp;action=move_up\">" . _("Up") . '</a>';
            }
            echo ']</small></td>';
        }
