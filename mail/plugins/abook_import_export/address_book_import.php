...
if (! sqGetGlobalVar('finish',$finish,SQ_POST)) {
    ...
    if (@move_uploaded_file($smusercsv['tmp_name'],$attachment_dir . $temp_file)) {
        ...
        if (count($csvdata) > 0) {
	    echo '<p class="infoimport"> ' . _("Is necessary nickname, first name and email to import") . '.</p>';
    ...
    if ($error) {
        echo '<br>'
	    . _("There were errors uploading the data, as listed below. Entries  listed here weren't uploaded.")
            . '<br><br> ' . $errorstring . '<br /> ';
    } else {
