...
function formatEnvheader($mailbox, $passed_id, $passed_ent_id, $message,
    ...
    if ($default_use_mdn) {
        if ($mdn_user_support) {
			...
			$smdn = html_entity_decode($mdn_url);
...
function formatMenubar($mailbox, $passed_id, $passed_ent_id, $message, $mbx_response) {
	...
	//$urlMailbox = str_replace("+"," ",$urlMailbox);
...
for ($i = 0; $i < $cnt; $i++) {
   ...
   if($default_charset == 'iso-8859-1'){
       ...
       $messagebody .= utf8_decode(formatBody($imapConnection, $message, $color, $wrap_at, $ent_ar[$i], $passed_id, $mailbox));
   }	
   if ($i != $cnt-1) {
